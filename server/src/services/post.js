...
export const plusExpired = ({ pid, days, status, eid }) => new Promise(async (resolve, reject) => {
    try {
        if (status === 'Accepted') {
            await db.Post.update({ expired: Date.now() + Number(days) * 24 * 3600 * 1000 }, {
                where: { id: pid }
            })
        }
        const response = await db.Expired.update({ status }, {
            where: { id: eid }
        })
        resolve({
            err: response ? 0 : 1,
            msg: response ? `Đã cập nhật.` : 'No post delete.',
        })
...
export const deleteExpired = (id) => new Promise(async (resolve, reject) => {
    try {
        const response = await db.Expired.destroy({ where: { id: id } })
        resolve({
            err: response > 0 ? 0 : 1,
            data: response > 0 ? 'Đã xóa' : 'Something went wrong',
        })
